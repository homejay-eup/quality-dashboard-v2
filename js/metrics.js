 ...
 *
 * 規則 D/E/G 的完整定義見 ../設備品質分析_分冊文件/00_規則定義.md（現行 v9）；
 * 本檔案只在對應計算處註明規則字母與改版日期，不重複抄規則全文。
 *
  ...
  function aggregate(rows, 上線量Enriched, selection, opts) {
    ...
    for (const r of filtered) {
      ...
      if (isReturned(r)) {
        ...
        // 規則 E 計算對象（../設備品質分析_分冊文件/00_規則定義.md，v9 現行）：
        // 不良品＋過保＋未歸類（＝非良品），且與其他三桶一致要求 isReturned
    ...
    const mkRow = (a, 上線量) => {
      ...
      const row = {
        ...
        '回廠良品數': o + qcQc + qcOth,
        '回廠不良品數': gg + x + ex,
        // 規則 D（../設備品質分析_分冊文件/00_規則定義.md，v7 現行）：D+E+回廠報廢，不含「其他(未過)」
        '回廠過保數': d + e + qcScrap,
        '回廠人為數': h,
