/**
 * js/transform.js — 清洗、條碼 join、分類（規則 A–G）
 *
 * 各規則完整定義與版本歷史的唯一來源：../設備品質分析_分冊文件/00_規則定義.md（現行 v9）。
 * 本檔案的區塊/函式註解只記「實作對應到哪條規則、哪次改版」與異動日期，不重複抄規則全文——
 * 改規則時先改那份文件的版本號，再回頭同步這裡的實作，避免兩邊各自漂移。
 *
...
App.transform = (() => {
  ...
  // → ../設備品質分析_分冊文件/00_規則定義.md 規則 A（v1 起未變）
  // ─────────────────────────────────────────────────
  ...
  // → ../設備品質分析_分冊文件/00_規則定義.md 規則 B（v1 起未變）
  // ─────────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────────
  // 規則 C：QC
  // → ../設備品質分析_分冊文件/00_規則定義.md 規則 C（v5 覆蓋後現行，之後版本未再變動）
  // ─────────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────────
  // 規則 G：四桶判斷（良品/不良品/過保/未歸類）
  // → ../設備品質分析_分冊文件/00_規則定義.md 規則 G（v7 新增三桶，v8 擴充第四桶「未歸類」）
  // ─────────────────────────────────────────────────

  /**
   * 依 QC + 維修分類 計算四桶布林標記。
   *
   * 2026-07-23 改版（使用者裁決，以三桶字面定義為準，取代舊版「不良品=QC全口徑」）：
   *   良品   = QC ∈ {廠商檢測正常, 回廠QC, 其他}
   *   不良品 = 維修分類 ∈ {G /評估後退修, X /已完修, 維修換貨＋換貨條碼}          （窄口徑，不含 D/E/H/V）
   *   過保   = 維修分類 ∈ {D /停產報廢, E /過保報廢} 或 QC = 回廠報廢              （不含 QC=其他(未過)）
   *   良品／不良品／過保 三桶互斥。
   *
   * 2026-07-24 新增第四桶（使用者裁決，讓四桶互斥且窮盡回廠量）：
   *   未歸類 = 不屬於良品／不良品／過保 的其餘情況，實際涵蓋：
   *     維修分類 H /人為報廢、V /已完修 人為、QC = 其他(未過)
   *   （已遺失另在 buildDetail() 覆寫良品/不良品/過保=false 後，一併強制 未歸類=true，見下方呼叫處）
   *   四桶互斥且加總＝回廠量，期間率（良品率+不良率+過保率+未歸類率）恆等於 100%。
   *
   * @param {string} QC
   * @param {string} 維修分類Code
   * @returns {{ 良品: boolean, 不良品: boolean, 過保: boolean, 未歸類: boolean }}
   */
  ...
  // ─────────────────────────────────────────────────
  // 規則 E：已使用年限（計算公式；「計算對象」哪些明細要平均在 js/metrics.js aggregate() 判定）
  // → ../設備品質分析_分冊文件/00_規則定義.md 規則 E（計算公式自 v3 起不變，v9 現行；
  //    v9 只改「計算對象」，不改這條公式本身）
  // ─────────────────────────────────────────────────

  /**
   * 規則 E：(品項完工日期 − 進貨日) / 365，取 1 位小數。
   ...
   *   良品(boolean)          不良品(boolean)        過保(boolean)        未歸類(boolean)
   *
  ...
  function buildDetail(raw, { year, quarter }) {
    ...
    const rows = cohort.map(row => {
      ...
      // ── 四桶（良品/不良品/過保/未歸類）
      let { 良品, 不良品, 過保, 未歸類 } = calcBuckets(QC, 維修分類Code);
      // 邊界決議（2026-07-17）：已遺失 計入回廠量（見 metrics），但不列入良品/不良品/過保
      //（遺失≠可沿用），避免虛增再使用率；歸為「未歸類」（2026-07-24 第四桶新增後強制歸類於此）。
      // 已升級維持依 QC 判定。
      if (回廠狀態 === '已遺失') {
        良品 = false;
        不良品 = false;
      ...
      // ── 維護原因（來源欄位，供車機/鏡頭分析範圍篩選：訊號異常／影像異常(鏡頭)）
      const 維護原因 = String(row['維護原因'] || '').trim();
