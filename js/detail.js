...
App.detail = (() => {
  ...
  const METRIC_BUCKET_COLS = {
    ...
    未歸類數: ['H /人為報廢', '其他(回廠)'],
    已使用年限: ['D /停產報廢', 'E /過保報廢', '回廠報廢', '回廠過保數', '其他(回廠)', '已使用年限'],
  };
  ...
  // 已使用年限：對齊 metrics.js aggregate() 的年限Sum/年限N 累加條件（有報廢資訊＝過保 或 QC='其他(未過)'，
  ...
  const METRIC_LOGIC = {
    ...
    已使用年限: {
      label: '已使用年限（有報廢資訊：過保或其他(未過) 且已使用年限不為空）',
      test: (r) => (r.過保 || r.QC === '其他(未過)') && r.已使用年限 != null,
    },
  };
