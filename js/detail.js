...
App.detail = (() => {
  ...
  const METRIC_BUCKET_COLS = {
    ...
    未歸類數: ['H /人為報廢', '其他(回廠)'],
    已使用年限: ['G /評估後退修', 'X /已完修', '維修換貨＋換貨條碼', '回廠不良品數', 'D /停產報廢', 'E /過保報廢', '回廠報廢', '回廠過保數', '其他(回廠)', '已使用年限'],
  };
  ...
  // 已使用年限：對齊 metrics.js aggregate() 的年限Sum/年限N 累加條件（非良品＝不良品或過保或未歸類，
  // 且已使用年限不為空；2026-07-29 起與其他三個一致要求 isReturned，避免「不回廠」明細混入年限平均）。
  const METRIC_LOGIC = {
    ...
    已使用年限: {
      label: '已使用年限（已回廠且非良品：不良品或過保或未歸類 且已使用年限不為空）',
      test: (r) => isReturned(r) && (r.不良品 || r.過保 || r.未歸類) && r.已使用年限 != null,
    },
  };
